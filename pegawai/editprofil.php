...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_FILES['foto_profil']['name'])) {
        ...
        move_uploaded_file($_FILES['foto_profil']['tmp_name'], __DIR__ . '/../uploads/' . $new);
...
.page-header{
  ...
  margin-bottom:25px;color:#1f3c88;
}
...
.card-title{
  ...
  color:#1f3c88;margin-bottom:25px;
}
.profile-img{text-align:center;margin-bottom:25px;}
.profile-img img{
  width:110px;height:110px;border-radius:50%;object-fit:cover;
}
/* keep input accessible to label clicks across browsers */
...
/* boxed wrapper so all fields have the same visible width */
.input-box{background:#fff;border:1px solid #e6e6e6;padding:8px 12px;border-radius:12px}
.skill-box{margin-top:25px;}
.skill-title{font-weight:700;margin-bottom:10px;}
.skill-list{display:grid;grid-template-columns:repeat(2,1fr);gap:10px}
.skill-item{display:flex;align-items:center;gap:10px;padding:6px 8px;border-radius:8px;background:transparent;color:#333;font-weight:600}
.skill-item input[type=checkbox]{width:16px;height:16px}
.skill-item .skill-pill{display:none}
.btn-group{
  ...
  <div class="card">
    ...
    <form method="post" enctype="multipart/form-data">
      <div class="profile-img">
        ...
        <label for="foto_profil" class="upload-btn">Pilih Foto</label>
      </div>

      <!-- LAYER KE-2 (SAMA DENGAN PROFILE.PHP) -->
      <div class="info-box">
        <table>
          <tr>
            <th>Nama</th>
            <td>
              <div class="input-box"><input type="text" name="nama" value="<?= htmlspecialchars($data['nama']) ?>"></div>
            </td>
          </tr>
          <tr>
            <th>Email</th>
            <td>
              <div class="input-box"><input type="email" name="email" value="<?= htmlspecialchars($data['email']) ?>"></div>
            </td>
          </tr>
          <tr>
            <th>Role</th>
          ...
          <tr>
            <th>NIP</th>
            <td>
              <div class="input-box"><input type="text" name="nip" value="<?= htmlspecialchars($data['nip']) ?>"></div>
            </td>
          </tr>
          <tr>
            <th>Role Humas</th>
          ...
          <tr>
            <th>Jabatan</th>
            <td>
              <div class="input-box"><input type="text" name="jabatan" value="<?= htmlspecialchars($data['jabatan']) ?>"></div>
            </td>
          </tr>
          <tr>
            <th>Nomor Telepon</th>
            <td>
              <div class="input-box"><input type="text" name="nomor_telepon" value="<?= htmlspecialchars($data['nomor_telepon']) ?>"></div>
            </td>
          </tr>
        </table>
      </div>
      ...
      <div class="skill-box">
        ...
        <div class="skill-list">
          ...
          foreach($labels as $f=>$l){
            ...
            echo "<label class='skill-item'><input type='checkbox' name='$f' $checked> <span class='skill-label'>".htmlspecialchars($l)."</span></label>";
          }
          ?>
        </div>
      </div>
...
<!-- Removed auto-submit on file select so user must click 'Simpan Perubahan' -->
