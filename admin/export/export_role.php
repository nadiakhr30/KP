...
$qData = mysqli_query($koneksi, "SELECT * FROM role ORDER BY nama_role");
$dataList = [];
...
if ($format == 'print') {
    ...
    <body>
        ...
        <div class="header-info">
            <h2>Laporan Data Role Sistem Kehumasan</h2>
        ...
        <table class="table table-striped">
            <thead>
                <tr>
                    ...
                    <th style="width: 85%;">Nama Role</th>
                </tr>
            </thead>
            <tbody>
                ...
                <tr>
                    <td><?= $index + 1 ?></td>
                    <td><?= $item['id_role'] ?></td>
                    <td><?= htmlspecialchars($item['nama_role']) ?></td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
    ...
    foreach ($dataList as $index => $item) {
        ...
        $sheet->setCellValue("B$row", $item['id_role']);
        $sheet->setCellValue("C$row", $item['nama_role']);
        $row++;
    }
    ...
    $filename = 'Laporan_Role_' . date('Y-m-d_H-i-s') . '.xlsx';
