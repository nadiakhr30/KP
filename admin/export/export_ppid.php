...
$qData = mysqli_query($koneksi, "SELECT * FROM ppid ORDER BY nama_ppid");
$dataList = [];
...
if ($format == 'print') {
    ...
    <body>
        ...
        <div class="header-info">
            <h2>Laporan Data PPID Sistem Kehumasan</h2>
        ...
        <table class="table table-striped">
            <thead>
                <tr>
                    ...
                    <th style="width: 85%;">Nama PPID</th>
                </tr>
            </thead>
            <tbody>
                ...
                <tr>
                    <td><?= $index + 1 ?></td>
                    <td><?= $item['id_ppid'] ?></td>
                    <td><?= htmlspecialchars($item['nama_ppid']) ?></td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
    ...
    $sheet->setTitle('Data PPID');
    
    $sheet->getColumnDimension('A')->setWidth(5);
    $sheet->getColumnDimension('B')->setWidth(10);
    $sheet->getColumnDimension('C')->setWidth(30);

    $headers = ['No', 'ID', 'Nama PPID'];
    ...
    foreach ($dataList as $index => $item) {
        ...
        $sheet->setCellValue("B$row", $item['id_ppid']);
        $sheet->setCellValue("C$row", $item['nama_ppid']);
        $row++;
    }
    ...
    $filename = 'Laporan_PPID_' . date('Y-m-d_H-i-s') . '.xlsx';
