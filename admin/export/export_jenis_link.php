...
// Get all data
$qData = mysqli_query($koneksi, "SELECT * FROM jenis_link ORDER BY nama_jenis_link");
$dataList = [];
...
if ($format == 'print') {
    ...
    <head>
        ...
        <title>Laporan Data Jenis Link</title>
    ...
    <body>
        ...
        <div class="header-info">
            <h2>Laporan Data Jenis Link Publikasi Sistem Kehumasan</h2>
        ...
        <table class="table table-striped">
            <thead>
                <tr>
                    ...
                    <th style="width: 85%;">Nama Jenis Link</th>
                </tr>
            </thead>
            <tbody>
                ...
                <tr>
                    <td><?= $index + 1 ?></td>
                    <td><?= $item['id_jenis_link'] ?></td>
                    <td><?= htmlspecialchars($item['nama_jenis_link']) ?></td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
...
// EXCEL FORMAT
    ...
    $sheet->setTitle('Data Jenis Link');
    
    $sheet->getColumnDimension('A')->setWidth(5);
    $sheet->getColumnDimension('B')->setWidth(10);
    $sheet->getColumnDimension('C')->setWidth(30);

    $headers = ['No', 'ID', 'Nama Jenis Link'];
    ...
    foreach ($dataList as $index => $item) {
        ...
        $sheet->setCellValue("B$row", $item['id_jenis_link']);
        $sheet->setCellValue("C$row", $item['nama_jenis_link']);
        $row++;
    }
    ...
    $filename = 'Laporan_Jenis_Link_' . date('Y-m-d_H-i-s') . '.xlsx';
