...
$qData = mysqli_query($koneksi, "SELECT * FROM jenis_pic ORDER BY nama_jenis_pic");
$dataList = [];
...
if ($format == 'print') {
    ...
    <head>
        ...
        <title>Laporan Data Jenis PIC</title>
    ...
    <body>
        ...
        <div class="header-info">
            <h2>Laporan Data Jenis PIC Sistem Kehumasan</h2>
        ...
        <table class="table table-striped">
            <thead>
                <tr>
                    ...
                    <th style="width: 85%;">Nama Jenis PIC</th>
                </tr>
            </thead>
            <tbody>
                ...
                <tr>
                    <td><?= $index + 1 ?></td>
                    <td><?= $item['id_jenis_pic'] ?></td>
                    <td><?= htmlspecialchars($item['nama_jenis_pic']) ?></td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
    ...
    $sheet->setTitle('Data Jenis PIC');
    
    $sheet->getColumnDimension('A')->setWidth(5);
    $sheet->getColumnDimension('B')->setWidth(10);
    $sheet->getColumnDimension('C')->setWidth(30);

    $headers = ['No', 'ID', 'Nama Jenis PIC'];
    ...
    foreach ($dataList as $index => $item) {
        ...
        $sheet->setCellValue("B$row", $item['id_jenis_pic']);
        $sheet->setCellValue("C$row", $item['nama_jenis_pic']);
        $row++;
    }
    ...
    $filename = 'Laporan_Jenis_PIC_' . date('Y-m-d_H-i-s') . '.xlsx';
