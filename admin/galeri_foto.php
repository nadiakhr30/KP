...
// Get sub_jenis from URL parameter, default to first one for jenis 'Galeri Foto'
$subId = isset($_GET['sub']) ? (int)$_GET['sub'] : 0;

// Fetch all sub_jenis for jenis 'Galeri Foto'
$qSub = mysqli_query($koneksi, "SELECT s.* FROM sub_jenis s JOIN jenis j ON s.id_jenis = j.id_jenis WHERE j.nama_jenis = 'Galeri Foto' ORDER BY s.nama_sub_jenis");
$allSub = [];
            ...
            <div class="row align-items-center">
                <div class="col-md-8">
                    <div class="page-header-title">
                        <h5 class="m-b-10">Manajemen Galeri Foto <?= $currentSub ? htmlspecialchars($currentSub['nama_sub_jenis']) : ''; ?></h5>
                        <p class="m-b-0">Untuk mengelola galeri foto <?= $currentSub ? htmlspecialchars($currentSub['nama_sub_jenis']) : ''; ?>.</p>
                    </div>
                </div>
                <div class="col-md-4">
                    <ul class="breadcrumb-title align-items-right">
                        ...
                        <li class="breadcrumb-item">
                            <a href="galeri_foto.php?sub=<?= $subId; ?>"><?= $currentSub ? htmlspecialchars($currentSub['nama_sub_jenis']) : 'Galeri Foto'; ?></a>
                        </li>
                    </ul>
                </div>
            </div>
                            ...
                            <div class="row users-card">
                                <?php if (count($dataMedia) === 0): ?>
                                    <div class="col-12 text-center">
                                        <p><?= $currentSub ? 'Tidak ada galeri foto untuk ' . htmlspecialchars($currentSub['nama_sub_jenis']) : 'Pilih sub jenis terlebih dahulu'; ?></p>
                                    </div>
                                <?php else: ?>
                                ...
                                <div class="dt-responsive table-responsive">
                                    <table id="order-table" class="table table-striped table-bordered nowrap">
                                        ...
                                        <tbody>
                                            <?php if (count($dataMedia) === 0): ?>
                                            <tr>
                                              <td colspan="6" class="text-center"><?= $currentSub ? 'Tidak ada galeri foto untuk ' . htmlspecialchars($currentSub['nama_sub_jenis']) : 'Pilih sub jenis terlebih dahulu'; ?></td>
                                            </tr>
                                            <?php else: ?>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document" style="max-width: 400px;">
        <div class="modal-content border-0 shadow-lg" style="border-radius: 15px; background: #fcf2f2; border-left: 5px solid #e74c3c;">
            <div class="modal-body" style="padding: 40px 30px; text-align: center;">
                ...
                <p style="font-size: 14px; color: #7f8c8d; margin-bottom: 20px;">Apakah Anda yakin ingin menghapus foto <strong id="deleteMediaName"></strong>?</p>
